...
int proc_copy_stack(kthread_t *dest, const kthread_t *src) {
    ...
    for(size_t i = 0; i < stack_size; i += 0x1000) {
        pgdir_map_page(
            ...
            (void *)(pgdir_get_page_entry((uint32_t *)dest->process->arch.cr3, (void *)(dest->arch.stack_end + i)) & 0xFFFFF000),
            (void *)(pgdir_get_page_entry((uint32_t *)dest->process->arch.cr3, (void *)(dest->arch.stack_end + i)) & 0xFFFFF000),
            0x03
        );

        memcpy(
            (void *)(pgdir_get_page_entry((void *)dest->process->arch.cr3, (void *)(dest->arch.stack_end + i)) & 0xFFFFF000),
...
int proc_copy_kernel_stack(kthread_t *dest, const kthread_t *src) {
    ...
    for(size_t i = 0; i < PROC_KERN_STACK_SIZE; i += 0x1000) {
        pgdir_map_page((uint32_t *)src->process->arch.cr3, (void *)(dest->arch.kernel_stack - PROC_KERN_STACK_SIZE + i), (void *)(dest->arch.kernel_stack - PROC_KERN_STACK_SIZE + i), 0x03);
    }
